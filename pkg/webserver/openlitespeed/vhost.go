...
func (v *baseVhost) loadSSL(cfg *Config) {
	...
	for _, d := range cfg.Directives("extraHeaders") {
		if strings.Contains(d.Value, "Strict-Transport-Security") {
			v.ssl.HSTS = true
		}
	}
...
var phpHandlerPattern = regexp.MustCompile(`^(?:fcgi|lsapi):\S*php(\d+)\s+php$`)
...
func (v *baseVhost) loadPHP(cfg *Config) {
	...
	for _, d := range sh.Directives("add") {
...
func (v *baseVhost) Save() error {
	...
	if err := os.WriteFile(filepath.Join(v.configDir, RegisterConfName), []byte(v.buildRegister().String()), 0600); err != nil {
		return fmt.Errorf("failed to save register file: %w", err)
	}
...
func (v *baseVhost) build() *Config {
	...
	acme.Add("addDefaultCharset", "off")
	acme.AddBlock("rewrite", "").Add("enable", "0")
...
func (v *baseVhost) buildPHP(cfg *Config) {
	...
	name := fmt.Sprintf("%s_php%d", v.safeName, v.php)
	ext := cfg.AddBlock("extprocessor", name)
	handler := "fcgi:" + name
	if LSAPIEnabled(v.php) {
		handler = "lsapi:" + name
		ext.Add("type", "lsapi")
		ext.Add("address", fmt.Sprintf("uds://tmp/lshttpd/%s.sock", name))
		ext.Add("maxConns", "35")
		ext.Add("env", "PHP_LSAPI_CHILDREN=35")
		ext.Add("env", "LSAPI_AVOID_FORK=200M")
		ext.Add("initTimeout", "60")
		ext.Add("retryTimeout", "0")
		ext.Add("persistConn", "1")
		ext.Add("pcKeepAliveTimeout", "60")
		ext.Add("respBuffer", "0")
		ext.Add("autoStart", "2")
		ext.Add("path", LSPHPPath(v.php))
		ext.Add("backlog", "100")
		ext.Add("instances", "1")
		ext.Add("extUser", "www")
		ext.Add("extGroup", "www")
		ext.Add("runOnStartUp", "3")
		ext.Add("priority", "0")
		ext.Add("memSoftLimit", "2047M")
		ext.Add("memHardLimit", "2047M")
		ext.Add("procSoftLimit", "1400")
		ext.Add("procHardLimit", "1500")
	} else {
		ext.Add("type", "fcgi")
		ext.Add("address", fmt.Sprintf("uds://tmp/php-cgi-%d.sock", v.php))
	...
	cfg.AddBlock("scripthandler", "").Add("add", handler+" php")
}
...
func (v *baseVhost) buildSSL(cfg *Config) {
	...
	b.Add("enableECDHE", "1")
	b.Add("renegProtection", "1")
	b.Add("sslSessionCache", "1")
	b.Add("sslSessionTickets", "1")
	b.Add("enableSpdy", "15")
	b.Add("enableQuic", map[bool]string{true: "1", false: "0"}[quic])
	if v.ssl.OCSP {
		b.Add("enableStapling", "1")
		b.Add("ocspRespMaxAge", "86400")
	}
	if v.ssl.HSTS {
		cfg.Add("extraHeaders", "Header set Strict-Transport-Security max-age=31536000")
	}
}
...
func (v *baseVhost) buildRewrite(cfg *Config, fragments []string) {
	...
	if v.ssl != nil && v.ssl.HTTPRedirect {
		...
		rw.Add("RewriteCond", "%{REQUEST_URI} !^"+strings.ReplaceAll(acmeURI, ".", `\.`))
...
func (v *baseVhost) buildRegister() *Config {
	cfg := &Config{}
	cfg.Append(&Comment{Text: " Auto-generated by AcePanel. DO NOT EDIT MANUALLY!"})
	b := cfg.AddBlock("virtualhost", v.siteName)
	b.Add("vhRoot", v.siteDir)
	b.Add("configFile", filepath.Join(v.configDir, VhostConfName))
...
func fragmentKind(content string) fragmentType {
	...
	for line := range strings.SplitSeq(content, "\n") {
		...
		if !strings.HasPrefix(strings.ToLower(line), "rewrite") {
