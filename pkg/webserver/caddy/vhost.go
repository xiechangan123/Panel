...
func (v *baseVhost) loadAuths(body *nodeList) {
	for _, d := range body.Directives("basic_auth") {
		...
		userFile := ""
		if imp := d.Directive("import"); imp != nil {
...
const accessLogFormat = `{request>remote_ip} - {request>user_id} [{ts}] "{request>method} {request>uri} {request>proto}" {status} {size} "{request>headers>Referer>[0]}" "{request>headers>User-Agent>[0]}"`
...
// safeName 将名称转换为 Caddyfile 标识符安全形式
