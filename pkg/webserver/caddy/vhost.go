...
type baseVhost struct {
	...
	domains   []string
	isDefault bool // 站点块额外带无主机名地址，承接未匹配任何域名的请求
	ssl       *types.SSLConfig
...
// siteKeys 每个监听端口与每个域名组合成一个显式带 scheme 的地址，按 scheme 分为两组；
// 无域名或默认站点时带无主机名地址，Caddy 会因此把该块当作兜底并排在带域名的块之后，主配置的内置兜底块在它之后
